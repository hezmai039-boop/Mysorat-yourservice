...
export default function Chat() {
  ...
  async function sendMessage(text: string, imageBase64?: string, imageMediaType?: string) {
    ...
    try {
      ...
      // Always clear first, even when this reply has no operationId - a
      // follow-up message that didn't create/match an operation must cancel
      // any navigation still pending from an earlier reply in the same
      // conversation, or the user gets silently redirected to a stale
      // operation from a few messages back after they've already moved on.
