...
4. Tell the user exactly: open `https://github.com/hezmai039-boop/Mysorat-yourservice/edit/main/<path>`, Ctrl+A, delete, paste, commit directly to `main`.
   - **For a brand-new file (a new migration folder, a new component, etc.), don't make the user type a nested path by hand.** GitHub's "create file" page accepts the target path and even the file's content as URL query parameters: `https://github.com/hezmai039-boop/Mysorat-yourservice/new/main?filename=<url-encoded path>&value=<url-encoded content>`. Build this URL yourself (Python's `urllib.parse.quote` on the path and on the content) and hand over one clickable link — the editor opens with the path already nested correctly and the content already in the box, so the user only has to glance at it and click "Commit new file". This removes the exact class of mistake where a hand-typed nested path becomes a stray sibling folder. Only fall back to plain path-typing instructions if the content is large enough to risk hitting URL length limits (rough rule of thumb: keep the built URL under ~6000 characters).
